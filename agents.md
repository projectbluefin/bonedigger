...
| [`clanker-queue`](docs/skills/clanker-queue.md) | Migrating queue labels to `clanker-queue/*`, syncing labels, or updating the rollout workflow |
...
- **bonedigger now scopes to report intake + confirm-driven priority escalation.** The clanker-queue rollout keeps label sync here; widget rendering and slash commands remain a downstream compatibility layer for now.
